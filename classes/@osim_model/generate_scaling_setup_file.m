%% generate Setup file for scaling
% 
% 
% 
%  input:
%       static_trc_name = 'statics_05.trc'
%       subject_number: 05
%       result_folder = '...\results'



function status = generate_scaling_setup_file(om, static_trc_name, subject_number, total_mass, end_time, result_folder)
...
subject_name = strcat('subject_',subject_number); % 'subject_05'

StaticName = static_trc_name;
OutputModelName = strcat(result_folder, '\scaled_',subject_name,'.osim'); % '...\result\scaled_subject_05.osim'
OutputMarkerModelName = strcat(result_folder, '\scaled_marker_',subject_name,'.osim');% '...\result\scaled_marker_subject_05.osim'
ScaleFileName = strcat(result_folder, '\scaled_setup_',subject_name,'.xml'); % 'scale_setup_05.xml';
ModelName = strcat('scaled_',subject_name,'.osim'); 
outputmotion_name = strcat(result_folder, '\',subject_name,'_scaling_motion.mot'); %'subject_05_scaling_motion.mot';
% output_scaleset_name = strcat(result_folder, '\',subject_name,'_scaleset_applied.xml');
output_scaleset_name = '';
maximal_time = end_time;
scaled_total_mass = total_mass;
...
% Scale_file.getGenericModelMaker().setModelFileName(char(om.model_path));
Scale_file.getGenericModelMaker().setMarkerSetFileName('markerfile.xml');


Scale_file.setName(ModelName);
Scale_file.setSubjectMass(scaled_total_mass);
...
Model_scaler.setOutputScaleFileName(output_scaleset_name);

Measurement_Set = Model_scaler.getMeasurementSet();
Measurement_Set.setName('modified MeasurementSet');
...
Scale_Set.setName('modified ScaleSet');
